...
### Dynamic Expertise (Skill Invocation)
When tasked with domain-specific work, you MUST NOT attempt to solve it using generic reasoning. Instead:
1.  **Consult the Catalog**: Refer to `.core/skills/index.md` to identify the correct specialist persona.
2.  **Adopt the Persona**: Transition into the identified specialist (e.g., `security-audit`, `ux`, `performance`).
3.  **Apply Specialized Protocols**: Use the specific rules and workflows defined in that skill's directory (e.g., `.core/skills/security-audit/`).
